...
class IonAuth
{
	...
	public function __construct()
	{
		...
		if (!$this->loggedIn() && get_cookie($this->config->item('identity_cookie_name', 'ion_auth')) && get_cookie($this->config->item('remember_cookie_name', 'ion_auth')))
		{
	...
	{
		if ( $this->ion_auth_model->forgottenPassword($identity) )   //changed
		{
	...
	public function forgottenPasswordComplete($code)
	{
		...
		$new_password = $this->ion_auth_model->forgottenPasswordComplete($code, $profile->salt);
	...
	public function forgottenPasswordCheck($code)
	{
		...
		else
		{
			if ($this->config->item('forgot_password_expiration', 'ion_auth') > 0) {
				...
				if (time() - $profile->forgottenPasswordTime > $expiration) {
					//it has expired
					$this->clear_forgottenPasswordCode($code);
					$this->set_error('password_change_unsuccessful');
					return FALSE;
				}
			}
	...
	public function isAdmin($id=false)
	{
		...
		return $this->inGroup($admin_group, $id);
	}
